...
const generateTokenAndSetCookie = (res, id, email, rememberMe = false) => {
...
export async function onboard(req, res) {
    ...
    const { full_name, age, gender, height, // cm
    weight, // kg
    fitness_goal, food_preference, region_preference, activity_level, add_egg_today, today_veg_only } = req.body;
    try {
        ...
        else if (activity_level === 'Professional Athlete')
            activityMultiplier = 2.1;
        else if (activity_level === 'Rehabilitation')
        ...
        if (fitness_goal === 'Muscle Gain')
            ...
            proteinPerKg = 1.9;
        else if (fitness_goal === 'Weight Loss')
            proteinPerKg = 1.8;
...
export async function logout(req, res) {
    ...
    try {
        if (userId && progressPercentage) {
         ...
         VALUES ($1, $2, $3, $4, $5)`, [logId, userId, 'logout_progress', `Logged out with today progress: ${percentage}%`, Math.round(percentage / 10)]);
