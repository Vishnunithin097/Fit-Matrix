...
export async function completeTask(req, res) {
    ...
    try {
        ...
        const lastCheck = await query(`SELECT completed_at FROM daily_tasks_log 
       WHERE user_id = $1 AND task_id = $2 
       ORDER BY completed_at DESC LIMIT 1`, [userId, taskId]);
        ...
        await query(`INSERT INTO daily_tasks_log (id, user_id, task_id, title, xp_value, completed_at) 
       VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP)`, [logId, userId, taskId, title, xpReward]);
        // Update user's total XP and streak status
        await query(`UPDATE users 
       SET xp = xp + $1, 
           current_streak = GREATEST(current_streak, 1) 
       WHERE id = $2`, [xpReward, userId]);
        // Fetch updated score
...
export async function logIntake(req, res) {
    const userId = req.user?.id;
    ...
    try {
        // Fetch current logs
        const result = await query('SELECT water_logged, calories_logged, protein_logged, carbs_logged, fats_logged, water_target, calorie_target, current_streak FROM users WHERE id = $1', [userId]);
       ...
       WHERE id = $6`, [newWater, newCalories, newProtein, newCarbs, newFats, userId]);
        ...
        if (achievedXp > 0) {
            for (const rew of rewards) {
                ...
                await query(`INSERT INTO daily_tasks_log (id, user_id, task_id, title, xp_value) 
           VALUES ($1, $2, $3, $4, $5) ON CONFLICT DO NOTHING`, [logId, userId, rew.key, rew.title, achievedXp]);
            }
...
export async function getProgressLogs(req, res) {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: 'Authentication required.' });
    }
    try {
        const result = await query(`SELECT * FROM daily_tasks_log 
       WHERE user_id = $1 
       ORDER BY completed_at DESC LIMIT 20`, [userId]);
        return res.status(200).json({
            tasks: result.rows
        });
    }
