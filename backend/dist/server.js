...
// ==========================================================================
app.post('/api/dashboard/food-profile', async (req, res) => {
    try {
        ...
        for (let i = 0; i < 7; i++) {
            const availableMeals = shuffledFoods.filter(item => !usedMeals.has(item.name));
            if (availableMeals.length === 0) {
                break;
            }
            const dayMeals = selectUnique(availableMeals, 3);
            dayMeals.forEach(item => usedMeals.add(item.name));
            const dayFeatureText = {
                day: i + 1,
                diet: calculatedDiet,
                cuisine: payload.cuisine || dayMeals[0]?.cuisine,
                fitness_goal: profile.fitness_goal,
                user_preference: profile.food_preference,
                meal_type: dayMeals.map(m => m.mealType).join(', '),
                meal_names: dayMeals.map(m => m.name).join(' | ')
            };
            const prediction = await scoreFoodItem(i + 1, dayFeatureText);
            const addOn = profile.addEggToday ? 'Boiled egg as a side for breakfast' : null;
            foodPlan.push({
                day: getFoodDayName(i),
                meals: dayMeals.map(m => ({ name: m.name, cuisine: m.cuisine, mealType: m.mealType })),
...
// ==========================================================================
app.post('/api/dashboard/gym-view', async (req, res) => {
    try {
        const payload = req.body;
        const profile = payload.userProfile;
        if (!profile) {
...
// ==========================================================================
app.post('/api/chatbot/nutrition-scan', async (req, res) => {
    try {
...
// ==========================================================================
app.post('/api/chatbot/product-image-reveal', upload.single('file'), async (req, res) => {
    try {
        if (!req.file) {
...
// ==========================================================================
app.post('/api/tasks/claim-xp', async (req, res) => {
    try {
