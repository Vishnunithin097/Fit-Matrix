...
function getBackupDataset(preference: 'Vegetarian' | 'Non-Vegetarian'): any[] {
  try {
...
export async function getPlan(req: Request, res: Response): Promise<any> {
  ...
  try {
    ...
    if (!mealsPlan || !workoutPlan) {
      ...
      await query(
        'UPDATE users SET meals_plan = $1, workout_plan = $2 WHERE id = $3',
        [JSON.stringify(mealsPlan), JSON.stringify(workoutPlan), userId]
      );
    }
...
export async function toggleDayMode(req: Request, res: Response): Promise<any> {
  const userId = req.user?.id;
...
export async function toggleDietRule(req: Request, res: Response): Promise<any> {
  const userId = req.user?.id;
...
function generateMealPlanHybrid(user: any) {
  ...
  if (user.weight < 50) calorieScale = 1.2;
  else if (user.weight > 80) calorieScale = 0.8;
  ...
  for (let d = 0; d < 7; d++) {
    ...
    mealTypes.forEach(mType => {
      ...
      let choices = dataset.filter((item: any) => item.mealType === mType);
      
      // Filter by Region if possible to give personalized feel, otherwise fallback
      let regionalChoices = choices.filter((item: any) => item.region === user.region_preference);
...
function applyMealToggles(mealsPlan: any, eggToday: boolean, vegOnlyToday: boolean, regionPref: string) {
  if (!mealsPlan || !mealsPlan.days) return mealsPlan;
  ...
  if (vegOnlyToday) {
    ...
    mealTypes.forEach(mType => {
      ...
      if (isNonVeg) {
        let choices = vegBackup.filter((v: any) => v.mealType === mType);
        let regionalChoices = choices.filter((v: any) => v.region === regionPref);
...
function generateWorkoutPlanHybrid(user: any) {
  ...
  const warmUp = [
    { exercise: "Dynamic Shoulder Rotations", sets: 2, reps: 15, duration: 3, caloriesBurned: 15, targetMuscle: "Shoulders/Rotator Cuff", difficulty: "Beginner" as const },
    { exercise: "Jumping Jacks (Cardio Activation)", sets: 3, reps: 30, duration: 4, caloriesBurned: 35, targetMuscle: "Full Body", difficulty: "Beginner" as const }
  ];
  ...
  if (user.weight < 50) {
    ...
    strength = [
      { exercise: "Barbell Squats", sets: 4, reps: 8, duration: 10, caloriesBurned: 110, targetMuscle: "Quadriceps/Glutes", difficulty: "Intermediate" as const },
      { exercise: "Dumbbell Bench Press", sets: 4, reps: 10, duration: 8, caloriesBurned: 90, targetMuscle: "Chest/Triceps", difficulty: "Intermediate" as const },
      { exercise: "Deadlifts (Traditional)", sets: 3, reps: 6, duration: 10, caloriesBurned: 140, targetMuscle: "Posterior Chain", difficulty: "Advanced" as const }
    ];
  } else if (user.weight > 80) {
    // Fat loss: high reps, metabolic circuits
    strength = [
      { exercise: "Bodyweight Goblet Squats", sets: 4, reps: 20, duration: 8, caloriesBurned: 95, targetMuscle: "Legs", difficulty: "Beginner" as const },
      { exercise: "Push-ups (Plank Form)", sets: 4, reps: 15, duration: 6, caloriesBurned: 70, targetMuscle: "Chest/Core", difficulty: "Beginner" as const },
      { exercise: "Kettlebell Swings", sets: 3, reps: 25, duration: 8, caloriesBurned: 120, targetMuscle: "Glutes/Hamstrings", difficulty: "Intermediate" as const }
    ];
  } else {
    // Maintenance / General conditioning
    strength = [
      { exercise: "Dumbbell Lunges", sets: 3, reps: 12, duration: 8, caloriesBurned: 80, targetMuscle: "Thighs/Glutes", difficulty: "Beginner" as const },
      { exercise: "Pull-ups (or Lat Pulldowns)", sets: 3, reps: 10, duration: 8, caloriesBurned: 85, targetMuscle: "Lats/Biceps", difficulty: "Intermediate" as const },
      { exercise: "Plank to Push-up Transitions", sets: 3, reps: 12, duration: 6, caloriesBurned: 60, targetMuscle: "Abdominals/Triceps", difficulty: "Intermediate" as const }
    ];
  }

  const cardio = user.weight > 80 ? [
    // Intense calorie burning
    { exercise: "High-Intensity Interval Sprinting (HIIT)", sets: 5, reps: 1, duration: 15, caloriesBurned: 210, targetMuscle: "Cardiovascular System", difficulty: "Intermediate" as const },
    { exercise: "Rowing Machine Intervals", sets: 3, reps: 1, duration: 10, caloriesBurned: 120, targetMuscle: "Total Body", difficulty: "Intermediate" as const }
  ] : [
    // Standard stamina building
    { exercise: "Incline Treadmill Jogging", sets: 1, reps: 1, duration: 15, caloriesBurned: 130, targetMuscle: "Legs/Cardio", difficulty: "Beginner" as const }
  ];

  const flexibility = [
    { exercise: "Hamstring Standing Fold", sets: 2, reps: 1, duration: 3, caloriesBurned: 10, targetMuscle: "Hamstrings", difficulty: "Beginner" as const },
    { exercise: "Cobra Yoga Stretch (Bhujangasana)", sets: 2, reps: 1, duration: 3, caloriesBurned: 12, targetMuscle: "Abdominals/Spine", difficulty: "Beginner" as const }
  ];

  const coolDown = [
    { exercise: "Static Quad Hold", sets: 2, reps: 1, duration: 3, caloriesBurned: 8, targetMuscle: "Quads", difficulty: "Beginner" as const },
    { exercise: "Mindful Deep Savasana", sets: 1, reps: 1, duration: 5, caloriesBurned: 5, targetMuscle: "Nervous System", difficulty: "Beginner" as const }
  ];
